...
//*************************************************************************
static void music_play_note_1(uint16_t note_index){//plays hit sound
    ...
    ece353_T32_2_wait(music_get_time_delay(Hit[note_index].time));
...
static void music_play_note_2(uint16_t note_index){//plays miss sound
    ...
    //wait given amount of time for the note to play
    //tells timer to wait time given by helper method
    ece353_T32_2_wait(music_get_time_delay(Miss[note_index].time));
...
static void music_play_note_3(uint16_t note_index){////plays win sound
    ...
    //wait given amount of time for the note to play
    //tells timer to wait time given by helper method
    ece353_T32_2_wait(music_get_time_delay(Win[note_index].time));
...
static void music_play_note_4(uint16_t note_index){//plays end game sound
    ...
    //wait given amount of time for the note to play
    //tells timer to wait time given by helper method
    ece353_T32_2_wait(music_get_time_delay(Open[note_index].time));
...
void music_play_song(uint8_t song){
    ...
    if(song == 3){
        for(i=0; i < 9; ++i){
           music_play_note_3(i); //win
        }
    }else if(song == 4){
        for(i=0; i < 10; ++i){
           music_play_note_4(i); //opening
        }
    }else{
        for(i=0; i < 2; ++i){
            if(song == 1){
                music_play_note_1(i); //Hit
            }else if(song == 2){
                music_play_note_2(i); //Miss
            }
        }
    }
}
